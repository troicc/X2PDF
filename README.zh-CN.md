...
<p align="center">
  <img src="docs/images/preview.png" alt="X2PDF 结构化预览界面" width="100%">
</p>

<p align="center">
  <a href="https://github.com/troicc/X2PDF/releases/latest"><strong>下载最新版本</strong></a> ·
  <a href="#安装">从源码安装</a> ·
  <a href="README.md">English</a>
</p>
...
<table>
<tr>
<td width="50%"><img src="docs/images/code-highlight.png" alt="代码高亮与复制按钮"></td>
<td width="50%"><img src="docs/images/formula-rendering.png" alt="可选公式与复制 LaTeX 按钮"></td>
</tr>
<tr>
<td align="center"><strong>可选择、可复制的高亮代码</strong></td>
<td align="center"><strong>原生 MathML 与 LaTeX 复制</strong></td>
</tr>
</table>

截图为按照实际预览样式制作的仓库演示图；具体导出效果取决于原始文章与排版选项。
...
打开 <https://github.com/troicc/X2PDF/releases/latest>，下载并解压 `X2PDF-v0.12.0.zip`。Release ZIP 只包含扩展运行文件。
...
排版偏好保存在 `chrome.storage.local`，下次预览会自动恢复。扩展界面会跟随浏览器 UI 语言，目前内置 English 与简体中文，其他语言回退为 English。
...
```text
X2PDF/
├── manifest.json               # Manifest V3 配置与本地化元数据
├── _locales/                   # English / 简体中文 Manifest 文案
├── background.js               # 扩展入口、网络捕获、临时标签页、PDF 下载
├── extractor.js                # DOM 兼容提取器与滚动采集
├── structured-parser.js        # X Article / DraftJS / entity 解析器
├── preview.html / preview.css  # 双语预览、阅读与打印样式
├── preview.js / i18n.js        # 文档渲染、设置、导出与界面本地化
...
├── scripts/                    # 校验、打包与 GitHub 发布脚本
├── docs/images/                # README 与社交预览图片
├── .github/                    # CI、Issue 表单与 PR 模板
├── package.json                # 可移植测试与打包命令
├── CHANGELOG.md
...
- Node.js 18+
- Python 3，用于打包和可选 PDF 回归测试
- Playwright 与 Poppler `pdftotext` 仅用于可选 PDF 文本层回归
...
扩展运行不需要构建步骤，修改后在 `chrome://extensions/` 中重新加载即可。
...
脚本会生成 `dist/X2PDF-v0.12.0.zip`，并排除测试、仓库文档和开发依赖。
...
测试内容包括 Manifest 与本地化校验、JavaScript 语法检查、DraftJS/Markdown/媒体/公式/剪贴板单元测试、PrismJS 高亮测试和 MathJax 转换测试。
...
GitHub Actions 会在推送到 `main`、Pull Request 与手动触发时自动测试并上传打包结果。`tests/native-mathml-pdf.test.py` 是可选 PDF 回归测试，需要 Chromium、Python Playwright 与 `pdftotext`。

## 发布流程

完整步骤见 [PUBLISHING.md](PUBLISHING.md)。完成 `gh auth login` 后，可一条命令完成测试、打包、提交、推送、仓库元数据设置、创建 Release 与上传 ZIP：

```bash
./scripts/publish-release.sh 0.12.0
```

Release 文案位于 [`docs/releases/v0.12.0.md`](docs/releases/v0.12.0.md)。将 [`docs/images/social-preview.png`](docs/images/social-preview.png) 上传到 GitHub 仓库设置中的 Social preview。
...
提交问题：<https://github.com/troicc/X2PDF/issues>
